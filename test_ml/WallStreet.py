...
def SharePriceOutputWithoutNews(index, inputSharePrice, timePoint):
	...
	newSharePrice = float(1)
	companyRegressionValues = RegressionCoeffList[index]

	if index == 3040:
		indexNew = ResetIndex(index, companyRegressionValues)
		index = indexNew

	# Obtaining some basic data
	actualPrice = companyRegressionValues(timePoint)
...
def ResetIndex(indexOld, regressionCoefficients):
	# here the code to sort out the problem in case the index runs out
	sharePriceLatest = regressionCoefficients(indexOld)
	for i in range(1,indexOld):
		sharePriceNew = regressionCoefficients(i)
		if sharePriceNew == sharePriceLatest:
			indexNew = indexOld
			return indexNew
		else:
			pass
